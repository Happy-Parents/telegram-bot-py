    ...
    if recipient_id:
        match text.lower():
            case txt if txt == FEEDBACK_QUESTION.lower():
                file_logger.info('Admin asked to rate support.')
                await context.bot.send_message(
                    chat_id=recipient_id,
                    text=FEEDBACK_REQUEST_MESSAGE,
                    reply_markup=FEEDBACK_OPTS_KEYBOARD
                )
                file_logger.info(f'User with id {recipient_id} was asked to rate support.')
            ...
            case _:
                ...
                file_logger.info(f'Admin unswer to user id {recipient_id} was sent.')
                await message.reply_text(ANSWER_SENT_MESSAGE)
    else:
        file_logger.error('Unable to unswer. User not found.')
